...
export default function NavDropdown({ item }) {
  return (
    <div className="relative group py-4 -my-4 z-[60]">
      <button className="hover:text-stone-900 transition-colors font-semibold cursor-pointer">{item.title}</button>
      
      {/* Dropdown panel */}
      <div className="absolute left-1/2 -translate-x-1/2 top-full pt-2 w-56 opacity-0 invisible pointer-events-none group-hover:opacity-100 group-hover:visible group-hover:pointer-events-auto transition-all duration-300 z-50 transform origin-top scale-95 group-hover:scale-100">
