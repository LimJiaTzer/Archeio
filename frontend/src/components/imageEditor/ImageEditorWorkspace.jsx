...
export default function ImageEditorWorkspace({
  ...
  const getActiveCropPercent = () =>
    appliedCropPercent ||
    initialCrop ||
    getFullCrop();

  const getCropPixelBounds = (
    cropPercent = getActiveCropPercent(),
    ...
    const safeCrop = cropPercent || getFullCrop();
    const left = (safeWidth * (Number(safeCrop.x) || 0)) / 100;
    const top = (safeHeight * (Number(safeCrop.y) || 0)) / 100;
    const width = Math.max(
      1,
      (safeWidth * (Number(safeCrop.width) || 100)) / 100
    );
    const height = Math.max(
      1,
      (safeHeight * (Number(safeCrop.height) || 100)) / 100
    );
  ...
  const constrainTextLayersToCrop = (
    layers,
    cropPercent = getActiveCropPercent()
  ) => {
    ...
    const cropBounds = getCropPixelBounds(cropPercent);
    const horizontalPadding = Math.min(
      cropBounds.width / 2,
      Math.max(8, cropBounds.width * 0.04)
    );
    const verticalPadding = Math.min(
      cropBounds.height / 2,
      Math.max(8, cropBounds.height * 0.04)
    );
    const availableTextWidth = Math.max(
      1,
      cropBounds.width - horizontalPadding * 2
    );
    const measurementContext =
      document.createElement('canvas').getContext('2d');

    return layers.map((layer) => {
      let fontSize = layer.fontSize || DEFAULT_TEXT_SIZE;
      const lines = (layer.text || '').split('\n');
      let fittedTextWidth = 0;

      if (measurementContext && layer.text?.trim()) {
        measurementContext.font = `700 ${fontSize}px ${
        ...
        if (widestLine > availableTextWidth) {
          fontSize = Math.max(
            8,
            fontSize * (availableTextWidth / widestLine)
          );
        }

        fittedTextWidth = Math.min(
          availableTextWidth,
          widestLine * (fontSize / (layer.fontSize || DEFAULT_TEXT_SIZE))
        );
      }
      ...
      const xPadding = Math.min(
        cropBounds.width / 2,
        Math.max(horizontalPadding, fittedTextWidth / 2)
      );
      const yPadding = Math.min(
        cropBounds.height / 2,
        Math.max(verticalPadding, textHeight / 2)
      );
      ...
      return {
        ...
        fontSize,
        x: Math.min(
          Math.max(layer.x, cropBounds.left + xPadding),
          cropBounds.right - xPadding
        ),
        y: Math.min(
          Math.max(layer.y, cropBounds.top + yPadding),
          cropBounds.bottom - yPadding
        ),
      };
    });
  };

  const storeTextLayers = (nextLayers) => {
...
const getFilterCss = (filterId) => {
  ...
  useEffect(() => {
    ...
    const frameId = requestAnimationFrame(updateCropViewportSize);
    const observer = new ResizeObserver(updateCropViewportSize);
  ...
  }, [isOpen, activeTool, presentation]);
  ...
  const addTextLayer = () => {
    ...
    const newLayer = {
      id: createLayerId(),
      text: '',
      x: cropBounds.left + cropBounds.width / 2,
      y: cropBounds.top + cropBounds.height / 2,
  ...
  const confirmTextLayer = (textId, textValue = null) => {
    ...
    const fittedLayers = constrainTextLayersToCrop(nextLayers);

    storeTextLayers(fittedLayers);
    setEditingTextId(null);
    ...
    setSelectedTextId(null);
    commitTextEditHistory(fittedLayers);
  };
  ...
  const pageCropDisplaySize =
    presentation === 'page' &&
    naturalImageSize.width > 0 &&
    naturalImageSize.height > 0 &&
    cropViewportSize.width > 0 &&
    cropViewportSize.height > 0
      ? (() => {
                      ...
                      >
                        <ReactCrop
                          ...
                          style={
                            pageCropDisplaySize
                              ? {
                                  width: `${pageCropDisplaySize.width}px`,
                                  height: `${pageCropDisplaySize.height}px`,
                                }
                              : undefined
                          }
                        >
                                ...
                                return (
                                  ...
                                  >
                                    {isEditingText ? (
                                      ...
                                      <span className="block cursor-move whitespace-pre-wrap break-words text-center">
                                        {layer.text}
                                      </span>
                                    )}
  ...
  if (presentation === 'page') {
    return isOpen ? editorContainer : null;
  }
